...
/*   Updated: 2024/12/06 11:54:06 by laoubaid         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	check_for_doors(t_val *val, int x, int y, int flag)
{
	...
	if (flag && corresponding_tile(*val, val->game->enemy_x, val->game->enemy_y) == 'D')
		return (1);
	else if (corresponding_tile(*val, val->game->plyr_x, val->game->plyr_y) == 'D')
		return (1);
	return (0);	
}
void apply_movement(t_val *val, double x, double y, int flag)
{
	...
	else if ((empty_space(*val, x, *y_corr) == '1' || check_for_doors(val, x, *y_corr, flag)))
		*x_corr = x;
	else if ((empty_space(*val, *x_corr, y) == '1' || check_for_doors(val, *x_corr, y, flag)))
		*y_corr = y;
}
